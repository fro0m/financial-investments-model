...
class RealtyObject(AssetBase):
  ...
  def instant_costs(self, by_month_end_index=1) -> int:
    ...
    elif month > months_to_renovation_start: # means that renovation has started       
      instant_costs = self.instant_price_rur + self.instant_price_renovation_rur
    elif month <= months_to_renovation_start: # means that renovation has not started
  ...
  def monthly_costs(self, by_month_end_index=1) -> int :
    month = by_month_end_index - 1
    if month > self.realty_mortgage_principal_and_interest_payments_months:
    ...
    if month > self.renovation_principal_and_interest_payments_months:
    ...
    result_in_current_month = (self.income_tax_percentage * self.monthly_income(month) + renovation_principal_and_interest_rur + \
    realty_mortgage_principal_and_interest_rur + (self.cap_ex_rur + \
    self.property_management_rur + self.insurance_rur + \
    self.additional_monthly_expenses_rur + self.utilities_rur) * cumulative_inflation_rate[month])

    return result_in_current_month

  def instant_income(self, by_month_end_index=1) -> int :
    month = by_month_end_index - 1
    return self.realty_object_price_rub * self.cumulative_realty_price_change_monthly_rate[month]

  def monthly_income(self, by_month_end_index=1) -> int :
    month = by_month_end_index - 1
    return (self.expected_monthly_rent_rur + self.additional_income_rur) * self.vacancy_percentage * \
      self.cumulative_realty_rent_change_monthly_rate[month]
...
def prepare_data_frame(data: np.array, number_of_months = number_of_months):
  months_range = range(len(data)) # Ensure the range matches the length of data
  return {'months_range': months_range, 'value': data}

import matplotlib.pyplot as plt

plt.matplotlib.use('Qt5Agg')
number_of_months_to_plot : int = 1000
avant_df_instant_costs = pd.DataFrame.from_dict(prepare_data_frame(avant_appartment.alltime_instant_costs(), number_of_months_to_plot))
avant_df_monthly_costs = pd.DataFrame.from_dict(prepare_data_frame(avant_appartment.alltime_cumulative_monthly_costs(), number_of_months_to_plot))
avant_df_instant_income = pd.DataFrame.from_dict(prepare_data_frame(avant_appartment.alltime_instant_income(), number_of_months_to_plot))
avant_df_monthly_income = pd.DataFrame.from_dict(prepare_data_frame(avant_appartment.alltime_cumulative_monthly_income(), number_of_months_to_plot))

fig, axes = plt.subplots(nrows=2, ncols=2, figsize=(8, 6))
axes = axes.flatten()  # Flatten the 2D array to 1D

# Plot a bar graph
avant_df_instant_costs.plot(ax=axes[0], x="months_range", y="value", kind="line")
avant_df_monthly_costs.plot(ax=axes[1], x="months_range", y="value", kind="line")
avant_df_instant_income.plot(ax=axes[2], x="months_range", y="value", kind="line")
avant_df_monthly_income.plot(ax=axes[3], x="months_range", y="value", kind="line")
