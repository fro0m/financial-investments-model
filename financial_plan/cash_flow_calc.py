...
cumulative_realty_rent_change_monthly_rate: np.array = np.full(number_of_months, 1 + 0.04/12) # https://www.perplexity.ai/search/srednii-rost-tseny-arendy-zhil-lhvgH9HnTGmuhlsJHdI6xg
cumulative_cash_conservative_investment_rate: np.array = np.full(number_of_months, 1 + 0.13/12) # basic rate of investment cash to a bank deposit or goverment bonds. This is used to adjust costs as they would be invested in the conservative tools
...
class AssetBase:
  ...
  def instant_costs(self, by_month_end_index=0) -> np.array:
    pass

  def monthly_costs(self, by_month_end_index=0) -> np.array :
    pass
  ...
  def instant_income(self, by_month_end_index=0) -> np.array :
    pass

  def monthly_income(self, by_month_end_index=0) -> np.array :
    pass
...
class RealtyObject(AssetBase):
  ...
  def instant_costs(self, by_month_end_index=0) -> np.array:
    result: np.array
    cumulative_result_in_last_month: int = 0
    for current_month in range(by_month_end_index):
      months_to_renovation_start = floor((self.date_of_getting_keys - date.today()).days / average_days_in_month)
      print(f'{months_to_renovation_start=}')
      if months_to_renovation_start < 0: # means that object already ready to use as current date is after date of building the realty
        instant_costs = self.instant_price_rur * self.cumulative_realty_price_change_monthly_rate[by_month_end_index] + self.instant_price_renovation_rur
      elif current_month > months_to_renovation_start:        
        instant_costs = self.instant_price_rur * self.cumulative_realty_price_change_monthly_rate[by_month_end_index] + self.instant_price_renovation_rur

      elif current_month >= 0 and current_month <= months_to_renovation_start:
      ...
      cumulative_result_in_last_month = (cumulative_result_in_last_month + instant_costs) * cumulative_cash_conservative_investment_rate[current_month]
  ...
  def monthly_costs(self, by_month_end_index=0) -> np.array :
    ...
    cumulative_result_in_last_month: int = 0
    for current_month in range(by_month_end_index):
      if current_month > self.realty_mortgage_principal_and_interest_payments_months:
      ...
      if current_month > self.renovation_principal_and_interest_payments_months:
      ...
      result_in_current_month = (self.taxes_percentage * self.monthly_income(current_month) + renovation_principal_and_interest_rur + \
      realty_mortgage_principal_and_interest_rur + (self.cap_ex_rur + \
      self.property_management_rur + self.insurance_rur + \
      self.additional_monthly_expenses_rur + self.utilities_rur) * cumulative_inflation_rate[current_month])

      cumulative_result_in_last_month = (cumulative_result_in_last_month + result_in_current_month) * cumulative_cash_conservative_investment_rate[current_month]
      result.append(cumulative_result_in_last_month)
    return result

  def instant_income(self, by_month_end_index=0) -> np.array :
    result: np.array
    for current_month in range(by_month_end_index):
      result.append(self.realty_object_price_rub * self.cumulative_realty_price_change_monthly_rate[current_month])
  ...
  def monthly_income(self, by_month_end_index=0) -> np.array :
    ...
    cumulative_result_in_last_month: int = 0
    for current_month in range(by_month_end_index):
      result_in_current_month = (self.expected_monthly_rent_rur + self.additional_income_rur) * self.vacancy_percentage * self.cumulative_realty_rent_change_monthly_rate[current_month]
      cumulative_result_in_last_month = cumulative_result_in_last_month * self.cumulative_realty_rent_change_monthly_rate[current_month] + result_in_current_month
      result.append(cumulative_result_in_last_month)
    return result
